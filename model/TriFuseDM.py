...
class MSD(nn.Module):
    ...
    def forward(self, x):
        ...
        return [rgb_feat, thermal_feat], [rgb_q, thermal_q]
...
class CrossIQ(nn.Module):
    ...
    def forward(self, x, s):
        ...
        scale = qr.size(1) ** 0.5
        attn_rgb = F.softmax((qr.transpose(1, 2) @ kt) / scale, dim=-1)
        attn_thermal = F.softmax((qt.transpose(1, 2) @ kr) / scale, dim=-1)
...
class RR(nn.Module):
    def __init__(self,in_ch,inc_ch):
        ...
        self.conv_out = nn.Conv2d(129,1,3,padding=1)
...
class TriFuse(nn.Module):
    ...
    def forward(self, rgb, t):
        ...
        x_refine = torch.cat(
            [x1_pred_vdt, self.up4(x1e[0] * x1s[0]), self.up4(x1e[1] * x1s[1])], dim=1)
        x_refine = self.rr(x_refine)

        return x1e_pred, x1e_pred_t, x_pred, x_refine
