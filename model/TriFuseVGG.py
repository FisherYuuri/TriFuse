...
from lib.vgg import vgg, a_vgg16
...
class RR(nn.Module):
    def __init__(self,in_ch,inc_ch):
        ...
        self.conv_out = nn.Conv2d(193,1,3,padding=1)
...
class TriFuse(nn.Module):
    def __init__(self,pretrained=False):
        ...
        self.msd4 = MSD(in_channels=512, out_channels=512)
    ...
    def forward(self, rgb, t, d):
        score_list_t = self.VGG(t)
        score_list_rgb = self.VGG(rgb)
        score_list_d = self.VGG(d)

        score_list_rgb = [F.interpolate(f, scale_factor=0.5, mode='bilinear', align_corners=False) for f in
                          score_list_rgb]
        score_list_t = [F.interpolate(f, scale_factor=0.5, mode='bilinear', align_corners=False) for f in score_list_t]
        score_list_d = [F.interpolate(f, scale_factor=0.5, mode='bilinear', align_corners=False) for f in score_list_d]

        x1 = [score_list_rgb[1],score_list_t[1],score_list_d[1]]
        x2 = [score_list_rgb[2],score_list_t[2],score_list_d[2]]
        x3 = [score_list_rgb[3],score_list_t[3],score_list_d[3]]
        x4 = [score_list_rgb[4],score_list_t[4],score_list_d[4]]
    ...
    def load_pretrained_model(self):
        st = torch.load("./pretrained/vgg16.pth")
        st2 = {}
        for key in st.keys():
            st2['base.' + key] = st[key]
        self.VGG.load_state_dict(st2)
