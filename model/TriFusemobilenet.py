...
class RR(nn.Module):
    def __init__(self,in_ch,inc_ch):
        ...
        self.conv_out = nn.Conv2d(73,1,3,padding=1)
...
class TriFuse(nn.Module):
    def __init__(self,pretrained=False):
        ...
        self.msd1 = MSD(in_channels=24, out_channels=24)
        self.msd2 = MSD(in_channels=40, out_channels=24)
        self.msd3 = MSD(in_channels=112, out_channels=40)
        self.msd4 = MSD(in_channels=160, out_channels=112)

        self.ciq1 = CrossIQ(in_channels=24, out_channels=48)
        self.ciq2 = CrossIQ(in_channels=24, out_channels=48)
        self.ciq3 = CrossIQ(in_channels=40, out_channels=48)
        self.ciq4 = CrossIQ(in_channels=112, out_channels=112)

        self.decoder1 = FASPP(in_channel=96, out_channel=48)
        self.decoder2 = FASPP(in_channel=96, out_channel=48)
        self.decoder3 = FASPP(in_channel=96, out_channel=48)
        self.decoder4 = FASPP(in_channel=112, out_channel=48)

        self.final_1 = nn.Sequential(
            Conv(24, 64, 3, bn=True, relu=True),
        ...
        self.final_4_vdt = nn.Sequential(
            ...
            Conv(48, 64, 3, bn=True, relu=True),
            Conv(64, 1, 1, bn=False, relu=False)
        )
        ...
        self.final_2_vdt = nn.Sequential(
            ...
            Conv(48, 32, 3, bn=True, relu=True),
    ...
    def forward(self, rgb, t, d):
        score_list_t = self.backbone(t)
        score_list_rgb = self.backbone(rgb)
        score_list_d = self.backbone(d)
    ...
    def load_pretrained_model(self):
        self.swin1.load_state_dict(torch.load('./pretrained/450_act3_mobilenetv3_large.pth',map_location='cpu'),strict=False)
