...
from lib.data_prefetcher import VMDataPrefetcher
import pytorch_iou
import pytorch_ssim
...
if __name__ == '__main__':
    ...
    img_root = './dataset/STERE/Train/'
    data = VDData(img_root)
    ...
    for epochi in tqdm(range(0, epoch + 1)):
        prefetcher = VMDataPrefetcher(loader)
        rgb, d, label = prefetcher.next()
        r_sal_loss = 0
        epoch_ave_loss = 0
        i = 0
        while rgb is not None:
            ...
            x1e_pred, x1e_pred_d, x_pred, x_refine = net(rgb, d)
            ...
            sal_loss = (loss0 * 2 + loss1 + loss3 + loss4 + loss5 + loss6 + loss7 + loss_x4eg + loss_x3eg + loss_x2eg + loss_x1eg + loss_x0eg * 2)
